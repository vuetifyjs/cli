# Vuetify CLI Monorepo

Scaffold and manage Vuetify projects with a suite of CLI tools.

## Packages

- `create-vuetify` — Scaffold a new Vuetify project.
- `create-vuetify-v0` — Scaffold a Vuetify project using the v0 presets.
- `@vuetify/cli` — Core CLI utilities and commands.
- `@vuetify/cli-shared` — Internal shared utilities (private).

## Development

<details>

<summary>Local development</summary>

- Install latest LTS [Node.js](https://nodejs.org/en/) and enable Corepack: `corepack enable`
- Install deps: `pnpm install`
- Build all packages: `pnpm build`

</details>

## License

MIT